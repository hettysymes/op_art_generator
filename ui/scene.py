from ui_old.nodes.nodes import Node

class AppState:

    def __init__(self, pos, zoom, save_states):
        self.pos = pos
        self.zoom = zoom
        self.save_states = save_states


class Scene:

    def __init__(self):
        self.states = {}

    def add(self, elem):
        self.states[elem.uid] = elem

    def get(self, uid):
        return self.states[uid]

    def remove(self, uid):
        del self.states[uid]


class NodeState:

    def __init__(self, uid, x, y, input_port_ids, output_port_ids, node: Node, svg_width,
                 svg_height, ignore_inputs=False, nodes_to_update=None, invisible=False, prop_ports=None):
        self.uid = uid
        self.x = x
        self.y = y
        self.input_port_ids = input_port_ids
        self.output_port_ids = output_port_ids
        self.node = node
        self.svg_width = svg_width
        self.svg_height = svg_height
        self.ignore_inputs = ignore_inputs
        self.nodes_to_update = nodes_to_update if nodes_to_update else []
        self.invisible = invisible
        self.prop_ports = prop_ports if prop_ports else []


class EdgeState:

    def __init__(self, uid, src_port_id, dst_port_id, invisible=False):
        self.uid = uid
        self.src_port_id = src_port_id
        self.dst_port_id = dst_port_id
        self.invisible = invisible


class PortState:

    def __init__(self, uid, x, y, parent_node_id, is_input, edge_ids, port_def):
        self.uid = uid
        self.x = x
        self.y = y
        self.parent_node_id = parent_node_id
        self.is_input = is_input
        self.edge_ids = edge_ids
        self.port_def = port_def
