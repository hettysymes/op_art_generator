...
def handle_port_ref_table(ref_data, port_ref_table, entry_class=PortRefTableEntry):
    ...
    for i, table_entry in enumerate(port_ref_table):
        ...
        if table_entry.ref in ref_data:
            # Ref already exists - do not add again but update ref data
            if table_entry.ref in ref_ids_to_add:
                ref_ids_to_add.remove(table_entry.ref)
            port_ref_table[i] = entry_class(table_entry.ref, table_entry.deletable, ref_data[table_entry.ref],
    ...
    return flatten_list(
        [entry.data if isinstance(entry, PortRefTableEntry) else entry for entry in port_ref_table]
    )
